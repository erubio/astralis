...
function App() {
  ...
  return <main>
    ...
    <section className="workspace">
      ...
      <section className="chart">{wheel ? <><div dangerouslySetInnerHTML={{ __html: wheel }}/><div className="chart-actions"><button className="zoom-button" type="button" onClick={() => setIsZoomed(true)}>Ampliar carta</button><button className="print-button" type="button" disabled={isInterpreting} onClick={() => setIsPrintDialogOpen(true)}>{isInterpreting ? "Preparando informe…" : "Exportar como PDF"}</button></div></> : <p className="chart-status">{error ?? "Calculando carta…"}</p>}{error && chart && <p className="error">{error}</p>}</section>
    ...
    {interpretation && <section className="print-interpretation-report" aria-hidden="true"><header><p className="print-brand">ASTRALIS · INTERPRETACIÓN DE LA CARTA</p><h1>{reportName}</h1><p>Lectura simbólica elaborada a partir de las posiciones y aspectos de esta carta natal.</p></header><div className="print-interpretation-sections">{interpretation.sections.map((section) => <article className="print-interpretation-section" key={section.id}><h2>{section.title}</h2><p>{section.text}</p></article>)}</div><footer>Generado con Astralis</footer></section>}
  </main>;
}
