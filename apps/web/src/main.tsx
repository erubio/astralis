  ...
  return <main>
    ...
    <section className="workspace">
      <aside><form onSubmit={(event) => { event.preventDefault(); void calculate(); }}>
        ...
        <label>Ciudad<input value={birthData.locationName} onChange={(event) => { setBirthData({ ...birthData, locationName: event.target.value }); setMatches([]); }}/></label>
        <button type="button" className="secondary" onClick={() => void searchLocations()} disabled={isSearching}>{isSearching ? "Buscando…" : "Buscar ciudad"}</button>
        {locationError && <p className="error">{locationError}</p>}
        {matches.length > 0 && <ul className="locations">{matches.map((location) => <li key={location.id}><button type="button" onClick={() => selectLocation(location)}>{location.label}<small>{location.timeZone}</small></button></li>)}</ul>}
        ...
        <button disabled={isLoading}>{isLoading ? "Calculando…" : "Calcular carta"}</button>
      </form></aside>
      <section className="chart">{wheel ? <div dangerouslySetInnerHTML={{ __html: wheel }}/> : <p className="chart-status">{error ?? "Calculando carta…"}</p>}{error && chart && <p className="error">{error}</p>}</section>
