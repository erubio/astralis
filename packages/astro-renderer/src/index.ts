...
export function renderNatalChartSvg(chart: NatalChart, size = 620, options: RendererOptions = {}): string {
  ...
  const zodiacLabels = zodiacDisplaySymbols.map((symbol, index) => text(point(index * 30 + 15, (outer + zodiacRing) / 2), symbol, "zodiac", `Signo ${index + 1}`, zodiacElements[index].color)).join("");
  ...
  const planets = placements.map((planet) => { const signIndex = Math.floor(planet.longitude / 30); const element = zodiacElements[signIndex]; return `<g data-planet="${planet.planet}">${text(point(planet.longitude, positionRadius(4)), planetSymbols[planet.planet], "planet", `${planet.planet} ${formatPosition(planet)}`)}${text(point(planet.longitude, positionRadius(3)), `${planet.degree}°`, "position-degree", `${planet.degree} grados`)}${text(point(planet.longitude, positionRadius(2)), zodiacDisplaySymbols[signIndex], "position-sign", planet.sign, element.color)}${text(point(planet.longitude, positionRadius(1)), `${String(planet.minute).padStart(2, "0")}′`, "position-minute", `${planet.minute} minutos`)}${planet.retrograde ? text(point(planet.longitude, positionRadius(0)), "℞", "retrograde", "Retrógrado") : ""}</g>`; }).join("");
