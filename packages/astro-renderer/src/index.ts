...
const zodiacSymbols = ["♈", "♉", "♊", "♋", "♌", "♍", "♎", "♏", "♐", "♑", "♒", "♓"];
const zodiacDisplaySymbols = zodiacSymbols.map((symbol) => `${symbol}︎`);
const zodiacElements = [
  { name: "fuego", color: "#e37b73", background: "#6e3443" }, { name: "tierra", color: "#83b57c", background: "#355842" },
  { name: "aire", color: "#d5b86b", background: "#625333" }, { name: "agua", color: "#70aac5", background: "#314f68" },
  { name: "fuego", color: "#e37b73", background: "#6e3443" }, { name: "tierra", color: "#83b57c", background: "#355842" },
  { name: "aire", color: "#d5b86b", background: "#625333" }, { name: "agua", color: "#70aac5", background: "#314f68" },
  { name: "fuego", color: "#e37b73", background: "#6e3443" }, { name: "tierra", color: "#83b57c", background: "#355842" },
  { name: "aire", color: "#d5b86b", background: "#625333" }, { name: "agua", color: "#70aac5", background: "#314f68" },
...
export function renderNatalChartSvg(chart: NatalChart, size = 620, options: RendererOptions = {}): string {
  ...
  const zodiacSector = (index: number) => {
    const start = index * 30; const end = start + 30;
    const startOuter = point(start, outer); const endOuter = point(end, outer);
    const endInner = point(end, zodiacRing); const startInner = point(start, zodiacRing);
    return `<path class="zodiac-sector zodiac-${zodiacElements[index].name}" fill="${zodiacElements[index].background}" d="M ${startOuter.x} ${startOuter.y} A ${outer} ${outer} 0 0 0 ${endOuter.x} ${endOuter.y} L ${endInner.x} ${endInner.y} A ${zodiacRing} ${zodiacRing} 0 0 1 ${startInner.x} ${startInner.y} Z"/>`;
  };
  const zodiacSectors = Array.from({ length: 12 }, (_, index) => zodiacSector(index)).join("");
  const zodiacLines = Array.from({ length: 12 }, (_, index) => radial(index * 30, zodiacRing, outer)).join("");
  const zodiacLabels = zodiacDisplaySymbols.map((symbol, index) => text(point(index * 30 + 15, (outer + zodiacRing) / 2), symbol, `zodiac zodiac-${zodiacElements[index].name}`, `Signo ${index + 1}`, zodiacElements[index].color)).join("");
  ...
  const planets = placements.map((planet) => { const signIndex = Math.floor(planet.longitude / 30); const element = zodiacElements[signIndex]; return `<g data-planet="${planet.planet}">${text(point(planet.longitude, positionRadius(4)), planetSymbols[planet.planet], "planet", `${planet.planet} ${formatPosition(planet)}`)}${text(point(planet.longitude, positionRadius(3)), `${planet.degree}°`, "position-degree", `${planet.degree} grados`)}${text(point(planet.longitude, positionRadius(2)), zodiacDisplaySymbols[signIndex], `position-sign zodiac-${element.name}`, planet.sign, element.color)}${text(point(planet.longitude, positionRadius(1)), `${String(planet.minute).padStart(2, "0")}′`, "position-minute", `${planet.minute} minutos`)}${planet.retrograde ? text(point(planet.longitude, positionRadius(0)), "℞", "retrograde", "Retrógrado") : ""}</g>`; }).join("");
  return `<svg viewBox="${-viewBoxPadding} ${-viewBoxPadding} ${size + viewBoxPadding * 2} ${size + viewBoxPadding * 2}" role="img" aria-label="Carta natal, ascendente ${formatDegrees(chart.houses.ascendant)}"><style>.ring{fill:none;stroke:#6a5e79;stroke-width:1.2}.zodiac-sector{opacity:.52}.divider{stroke:#a59ab5;stroke-width:.8}.aspect{fill:none;stroke-width:1.2;opacity:.78}.aspect-symbol{font:17px Georgia;text-anchor:middle;dominant-baseline:middle}.planet{fill:#f4c872;font:29px Georgia;text-anchor:middle;dominant-baseline:middle}.position-degree,.position-minute{fill:#b5a6c4;font:12px system-ui;text-anchor:middle;dominant-baseline:middle}.position-sign{font:22px Georgia;text-anchor:middle;dominant-baseline:middle}.retrograde{fill:#f4c872;font:16px Georgia;text-anchor:middle;dominant-baseline:middle}.zodiac{font:27px Georgia;text-anchor:middle;dominant-baseline:middle}.house{fill:#d9d0e1;font:16px system-ui;text-anchor:middle;dominant-baseline:middle}.cusp{fill:#b5a6c4;font:10px system-ui;text-anchor:middle;dominant-baseline:middle}.outer-cusp{fill:#f4c872;font:700 11px system-ui;text-anchor:middle;dominant-baseline:middle}.outer-angle{fill:#f4c872;font:700 12px system-ui;text-anchor:middle;dominant-baseline:middle}</style><g>${zodiacSectors}</g><circle class="ring" cx="${center}" cy="${center}" r="${outer}"/><circle class="ring" cx="${center}" cy="${center}" r="${zodiacRing}"/><circle class="ring" cx="${center}" cy="${center}" r="${houseRing}"/><circle class="ring" cx="${center}" cy="${center}" r="${aspectRing}"/><g class="divider">${zodiacLines}${houseLines}${radial(chart.houses.ascendant, houseRing, outer)}${radial(chart.houses.midheaven, houseRing, outer)}</g>${aspects}${zodiacLabels}${houseLabels}${cuspLabels}${planets}${outerCuspLabels}${outerAngleLabels}<circle cx="${center}" cy="${center}" r="3" fill="#f4c872"/></svg>`;
}

function text(point: Point, value: string, className: string, label: string, fill?: string): string {
  return `<text x="${point.x}" y="${point.y}" class="${className}"${fill ? ` fill="${fill}"` : ""}${label ? ` aria-label="${label}"` : ""}>${value}</text>`;
}
