...
export function renderNatalChartSvg(chart: NatalChart, size = 620, options: RendererOptions = {}): string {
  ...
  return `<svg viewBox="${-viewBoxPadding} ${-viewBoxPadding} ${size + viewBoxPadding * 2} ${size + viewBoxPadding * 2}" role="img" aria-label="Carta natal, ascendente ${formatDegrees(chart.houses.ascendant)}"><style>.ring{fill:none;stroke:#6a5e79;stroke-width:1.2}.divider{stroke:#a59ab5;stroke-width:.8}.aspect{fill:none;stroke-width:1.2;opacity:.78}.aspect-symbol{font:15px Georgia;text-anchor:middle;dominant-baseline:middle}.planet{fill:#f4c872;font:24px Georgia;text-anchor:middle;dominant-baseline:middle}.position-degree,.position-minute{fill:#b5a6c4;font:10px system-ui;text-anchor:middle;dominant-baseline:middle}.position-sign{fill:#c5b5d8;font:18px Georgia;text-anchor:middle;dominant-baseline:middle}.retrograde{fill:#f4c872;font:13px Georgia;text-anchor:middle;dominant-baseline:middle}.zodiac{fill:#c5b5d8;font:24px Georgia;text-anchor:middle;dominant-baseline:middle}.house{fill:#d9d0e1;font:15px system-ui;text-anchor:middle;dominant-baseline:middle}.cusp{fill:#b5a6c4;font:9px system-ui;text-anchor:middle;dominant-baseline:middle}.outer-cusp{fill:#f4c872;font:700 10px system-ui;text-anchor:middle;dominant-baseline:middle}.outer-angle{fill:#f4c872;font:700 11px system-ui;text-anchor:middle;dominant-baseline:middle}</style><circle class="ring" cx="${center}" cy="${center}" r="${outer}"/><circle class="ring" cx="${center}" cy="${center}" r="${zodiacRing}"/><circle class="ring" cx="${center}" cy="${center}" r="${houseRing}"/><circle class="ring" cx="${center}" cy="${center}" r="${aspectRing}"/><g class="divider">${zodiacLines}${houseLines}${radial(chart.houses.ascendant, houseRing, outer)}${radial(chart.houses.midheaven, houseRing, outer)}</g>${aspects}${zodiacLabels}${houseLabels}${cuspLabels}${planets}${outerCuspLabels}${outerAngleLabels}<circle cx="${center}" cy="${center}" r="3" fill="#f4c872"/></svg>`;
}
